...
that wants to post a Nostr note through Omostrich, on behalf of
whoever installed and runs this on their own machine. It does not
implement anything itself — the actual posting logic lives entirely in
this repo's daemon (`daemon/daemon.mjs`) and its two client surfaces:
...
1. **Post only when the user explicitly asks, in that exact moment,
   for that exact text.** Not "they mentioned wanting to post
   something earlier," not "this seems like something they'd want
   posted," not as a side effect of some other task you were doing. If
   they didn't ask you, right now, to post this, don't call `publish`.
...
3. **Never invent content the user didn't ask you to post.** Don't
   summarize, don't embellish, don't "improve" the wording unless they
   specifically asked you to draft something and then separately
   confirmed posting it. What gets published should be recognizably
   what they asked for.

4. **Never attempt to unlock the vault.** If `status` shows `locked:
   true`, or `publish` comes back with `{"ok":false,"error":"locked"}`,
   tell the user the signer is locked and to unlock it themselves from
   the Omostrich bar icon (the ostrich). No agent has, or should ever
...
6. **Never print, log, or otherwise surface an nsec or passphrase** in
   any output, transcript, or message to the user or anyone else. This
...
(some relays succeeded, some didn't) is still worth telling the user
about honestly rather than rounding up to "done."
